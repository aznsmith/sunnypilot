...
def _initialize_mazda(CP: structs.CarParams, params_dict: dict[str, str]) -> None:
  if CP.brand == 'mazda':
    torque_interceptor = int(params_dict.get("MazdaTorqueInterceptorEnabled", 0)) == 1
    if torque_interceptor:
      ...
      # The interceptor removes the stock steering lockout -- see report.md / the mazda.h
      # review for the mechanism (torque sensor + LKAS command both routed through the
      # interceptor device rather than the stock EPS path).
