...
try:
  ...
  class Params:  # type: ignore[no-redef]
    ...
    def get(self, key, encoding=None):
      try:
        return self._p.get(key, encoding=encoding)
      except Exception:
        return None
    def get_bool(self, key):
      try:
        return self._p.get_bool(key)
      except Exception:
        return _params_file_get_bool(key)
except ImportError:
  class Params:  # type: ignore[no-redef]
    def get(self, key, encoding=None): return None
    def get_bool(self, key): return _params_file_get_bool(key)
...
class CarInterface(CarInterfaceBase):
  ...
  def get_lataccel_torque_siglin(self) -> tuple:
  ...
  def torque_from_lateral_accel(self) -> TorqueFromLateralAccelCallbackType:
    if self.CP.carFingerprint in NON_LINEAR_TORQUE_PARAMS:
      ...
      def torque_from_lateral_accel_siglin(lateral_acceleration: float, torque_params: structs.CarParams.LateralTorqueTuning, gravity: bool = False) -> float:
        return float(np.interp(lateral_acceleration, lataccel_values, torque_values))
      return torque_from_lateral_accel_siglin
    else:
      return self.torque_from_lateral_accel_linear  # type: ignore[return-value]

  def lateral_accel_from_torque(self) -> LateralAccelFromTorqueCallbackType:
    if self.CP.carFingerprint in NON_LINEAR_TORQUE_PARAMS:
      torque_values, lataccel_values = self.get_lataccel_torque_siglin()

      def lateral_accel_from_torque_siglin(torque: float, torque_params: structs.CarParams.LateralTorqueTuning, gravity: bool = False) -> float:
        return float(np.interp(torque, torque_values, lataccel_values))
      return lateral_accel_from_torque_siglin
    else:
      return self.lateral_accel_from_torque_linear  # type: ignore[return-value]
  ...
  @staticmethod
  def _get_params_sp(stock_cp: structs.CarParams, ret, candidate, fingerprint: dict[int, dict[int, int]],
                     car_fw: list[structs.CarParams.CarFw], alpha_long: bool, is_release_sp: bool, docs: bool):
